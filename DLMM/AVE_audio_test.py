...
ResNet.load_state_dict(torch.load('/hdd/DLMM/code/AVE/model/retrain/retrain_audio_net1_003.pth'))
Prototype.load_state_dict(torch.load('/hdd/DLMM/code/AVE/model/retrain/retrain_audio_net2_003.pth'))
MLP_Classifier.load_state_dict(torch.load('/hdd/DLMM/code/AVE/model/retrain/retrain_audio_net3_003.pth'))
ResNet.eval()
Prototype.eval()
MLP_Classifier.eval()
...
if __name__ == "__main__":
    ...
    for epoch in range(pre_epoch, EPOCH):
        ...
        print('声音模态增量学习后，在目标域测试分类准确率为：%f' % (acc))
        

        testall_img = np.array(testall_img)
        np.save('/hdd/DLMM/code/AVE/savefile/audio_retrain_distance.npy',testall_img) #存为npy文件，后续利用
