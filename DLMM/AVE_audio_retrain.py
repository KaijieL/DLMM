...
if __name__ == "__main__":
    ...
    for epoch in range(pre_epoch, EPOCH):
        ...
        union_all = union_all[0 : int(0.3*len(union_all))]
        ...
        for element in union_all:
            ...
            retrain_audioloss.append(float(loss))       #对模态课程学习排序用
        ...
        percent = min( int ( epoch / 1 +1) * 0.23, 1 ) #每1个epoch增加20%的样本，由易到难课程学习
        ...
        for i in range(trainbatch):
            ...
            for dir_img in file_list_current:  
                try:
                    ...
                    image = np.loadtxt('/hdd/DLMM/dataset/AVE/all_audio/'+dir_img[2])
                    image = torch.from_numpy(image)
                    image = image.reshape(1,1,64,64)
        ...
        #保存模型
        torch.save(ResNet.state_dict(), '/hdd/DLMM/code/AVE/model/retrain/retrain_audio_net1_%03d.pth' % (epoch + 1))
        torch.save(Prototype.state_dict(), '/hdd/DLMM/code/AVE/model/retrain/retrain_audio_net2_%03d.pth' % (epoch + 1))
        torch.save(MLP_Classifier.state_dict(), '/hdd/DLMM/code/AVE/model/retrain/retrain_audio_net3_%03d.pth' % (epoch + 1))
